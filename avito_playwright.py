...
def collect_ads_links(page):
  ...
  links = []
  # Первый тип
  elems1 = page.query_selector_all('a[href][data-marker="item-title"]')
  for el in elems1:
...
def infinite_scroll_with_next(page, existing_links, max_links=200):
  ...
  while True:
    ...
    if total_new == old_count:
      next_link_el = page.query_selector(
...
def main():
  user_id = input(
      "Введите уникальный user_id и запомните его (используется для параллельных запусков и отображается в названии окна)?: ")

  MAIN_FILE = f"{user_id}/ads_links.txt"  # Основной файл со ссылками на объявления
  SELLER_IDS_FILE = f"{user_id}/seller_ids.txt"  # Файл c ID продавцов
  CURRENT_LINK_FILE = f"{user_id}/current_link.txt"  # Файл с текущей (последней обработанной) ссылкой

  # Создадим папку под пользователя
  ...
  max_links_str = input(
      "Введите максимальное число ссылок для сбора за один раз [по умолчанию 200]: ").strip()
  try:
  ...
  messages_count_str = input(
      "Сколько всего сообщений нужно отправить? [по умолчанию 10]: ") or "10"
  min_views_str = input(
      "Минимальное число просмотров? [0 = не проверять]: ") or "0"
  synonyms_text = input(
      "Введите словарь синонимов (формат 'слово: синоним1, синоним2; ...'): ") or ""
  original_message = input(
      "Введите исходный текст сообщения (можно использовать \\n для переноса строк): ")
